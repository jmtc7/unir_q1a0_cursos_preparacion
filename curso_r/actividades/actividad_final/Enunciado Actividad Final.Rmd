...
edad_media_compradores <- bikes %>%
  ...
  summarise(Edad_Media = mean(Age, na.rm = TRUE))

edad_media_no_compradores <- bikes %>%
  filter(Purchased.Bike == "No") %>%
  summarise(Edad_Media = mean(Age, na.rm = TRUE))
...
```{r}
bikes$Age_Group <- cut(bikes$Age, breaks = c(0, 29, 59, Inf),
...
cor_no_compradores <- cor(bikes[bikes$Purchased.Bike == "No",]$Cars, 
                       bikes[bikes$Purchased.Bike == "No",]$Income, use = "complete.obs")

print(cor_total)
print(cor_compradores)
print(cor_no_compradores)
```
<hr>
<hr>
